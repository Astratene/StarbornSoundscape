...
class ConeRenderer(context: EntityRendererFactory.Context?) :
    EntityRenderer<ConeRendererEntity>(context) {

    override fun render(
        entity: ConeRendererEntity,
        yaw: Float,
        tickDelta: Float,
        ...
        val cubes = entity.dataTracker.get(ConeRendererEntity.InnerCubes)
        val colour: Int = entity.dataTracker.get(ConeRendererEntity.OuterColour)
        val thickness = entity.dataTracker.get(ConeRendererEntity.OuterThickness)
        val innerColour: Int = entity.dataTracker.get(ConeRendererEntity.InterColour)
        ...
        val opacity: Float = entity.dataTracker.get(ConeRendererEntity.Opacity)
        val buffer = vertexConsumers.getBuffer(RenderLayer.getLightning())
        val thicknessMod = entity.dataTracker.get(ConeRendererEntity.EndSize)

        renderEnds(entity, matrices, buffer, thickness * 2f, outerRed, outerGreen, outerBlue, opacity)

        val pos = entity.dataTracker.get(ConeRendererEntity.OriginPos).toVec3d()
        val targetPos = entity.dataTracker.get(ConeRendererEntity.TargetPos).toVec3d()
    ...
    fun renderEnds(
        ...
        val modifiedDistance =
            distance.toFloat() + (if (entity.dataTracker.get(ConeRendererEntity.MaxOuterThickness) > 1f) 1f else 0f)
...
//        buffer.xyz(matrices.peek(), Vector3f(0f - thicknessMod, modifiedDistance, thickness + thicknessMod)).color(red, green, blue, opacity)
    ...
    override fun shouldRender(entity: ConeRendererEntity?, frustum: Frustum?, x: Double, y: Double, z: Double): Boolean {
        return true
    }


    override fun getTexture(entity: ConeRendererEntity): Identifier {
