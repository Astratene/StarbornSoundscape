...
import org.teamvoided.starborn_soundscape.init.StarbornSoundscapeEntities.BEAM_RENDERER

class BeamRendererEntity : Entity {

    constructor(entityType: EntityType<out BeamRendererEntity?>?, world: World?) :
            super(entityType as EntityType<out Entity?>?, world)

    constructor(world: World?, owner: LivingEntity?) :
            super(BEAM_RENDERER as EntityType<out Entity?>, world)
    ...
    companion object {
        val InterColour: TrackedData<Int> =
            DataTracker.registerData(BeamRendererEntity::class.java, TrackedDataHandlerRegistry.INTEGER)
        val OuterColour: TrackedData<Int> =
            DataTracker.registerData(BeamRendererEntity::class.java, TrackedDataHandlerRegistry.INTEGER)
        val TargetPos: TrackedData<Vector3f> =
            DataTracker.registerData(BeamRendererEntity::class.java, TrackedDataHandlerRegistry.VECTOR3F)
        val MaxOuterThickness: TrackedData<Float> =
            DataTracker.registerData(BeamRendererEntity::class.java, TrackedDataHandlerRegistry.FLOAT)
